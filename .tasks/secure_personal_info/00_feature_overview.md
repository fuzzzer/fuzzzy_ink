...
Fuzzy Vault is a new section of Fuzzzy Ink that provides a **fully offline, encrypted personal information manager**. It has two primary capabilities:
...
| **Offline-Only** | Zero network. Zero servers. Same constraint as the rest of Fuzzzy Ink. |
...
Fuzzy Vault follows the exact same architecture as `fuzzzy_ink/` and `fuzzy_auth/`:
